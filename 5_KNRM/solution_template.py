...
class GaussianKernel(torch.nn.Module):
    ...
    def forward(self, x):
        # 3 допишите ваш код здесь
        distances = ((x[:, :, None]  - self.mu) ** 2)
        kernel_matrix = torch.exp(-distances / (2 * self.sigma ** 2)).sum(-1)
...
class KNRM(torch.nn.Module):
    ...
    def _get_kernels_layers(self) -> torch.nn.ModuleList:
        ...
        # 3 допишите ваш код здесь
        kernel_list = np.concatenate([[val for i, val in enumerate(np.linspace(-1, 1, num=2*self.kernel_num-1)) if i%2==1], [1]])
        for mu in kernel_list:
            kernel = GaussianKernel(mu=mu, sigma=self.sigma)
            if mu==1:
                print('mu==1')
                kernel = GaussianKernel(mu=mu, sigma=self.exact_sigma)
    ...
    def _get_mlp(self) -> torch.nn.Sequential:
       # 3 допишите ваш код здесь
        ...
        if len(self.out_layers) == 0:
            modules.append(torch.nn.Linear(self.kernel_num, 1))
        else:
            i_prev = self.kernel_num
    ...
    def _get_matching_matrix(self, query: torch.Tensor, doc: torch.Tensor) -> torch.FloatTensor:
        ...
        query_emb = torch.stack([
            torch.stack([self.embeddings.weight[idx] for idx in q]) 
            for q in query
        ])
        doc_emb = torch.stack([
            torch.stack([self.embeddings.weight[idx] for idx in d])
            for d in doc
        ])
        matching_matrix = F.cosine_similarity(query_emb[:,:,None,:], doc_emb[:,None,:,:], dim=-1) 
...
class RankingDataset(torch.utils.data.Dataset):
    ...
    def _tokenized_text_to_index(self, tokenized_text: List[str]) -> List[int]:
        # 4 допишите ваш код здесь
        result = [self.vocab[word] if word in self.vocab else self.oov_val for word in tokenized_text]
        return (result + [self.vocab['PAD']] * self.max_len)[:self.max_len]

    def _convert_text_idx_to_token_idxs(self, idx: int) -> List[int]:
        # 4 допишите ваш код здесь
        tokenized_text = self.idx_to_text_mapping[idx]
        tokenized_text = self.preproc_func(tokenized_text)
...
class ValPairsDataset(RankingDataset):
    def __getitem__(self, idx):
        # 4 допишите ваш код здесь
        result = self.index_pairs_or_triplets[idx]
        query = self._convert_text_idx_to_token_idxs(result[0])
        document = self._convert_text_idx_to_token_idxs(result[1])
        return {'query': query, 'document': document}, result[2]
...
class Solution:
    ...
    def get_glue_df(self, partition_type: str) -> pd.DataFrame:
        ...
        glue_df_fin = pd.DataFrame({
            'id_left': glue_df['qid1'].astype(int),
            'id_right': glue_df['qid2'].astype(int),
    ...
    def simple_preproc(self, inp_str: str) -> List[str]:
        ...
        out_str = self.hadle_punctuation(out_str)
    ...
    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        ...
        union_df['words_list'] = union_df['text'].apply(lambda x: self.simple_preproc(x))
        vocab = pd.Series([word for doc in union_df['words_list'] for word in doc])
        vocab = vocab.value_counts().to_dict()
        filtered_vocab = self._filter_rare_words(vocab=vocab, min_occurancies=min_occurancies)
        return list(filtered_vocab.keys())

    def _read_glove_embeddings(self, file_path: str) -> Dict[str, List[str]]:
        # 2 допишите ваш код здесь
        glove_embeddings = {}
    ...
    def create_glove_emb_from_file(self, file_path: str, inner_keys: List[str],
        ...
        matrix = np.concatenate((mask_matrix, unk_words_matrix, matrix))
